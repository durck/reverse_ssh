...
func main() {
	...
	server.Run(listenAddress, dataDir, connectBackAddress, autogeneratedConnectBack, tlscert, tlskey, insecure, enabledDownloads, tls, openproxy, timeout, wsPath, pushPath, trustedProxyCIDRs)
}
	...
	for _, value := range values {
		for _, part := range strings.Split(value, ",") {
			...
			if err != nil {
				ip := net.ParseIP(part)
				if ip == nil {
					return nil, fmt.Errorf("invalid --trusted-proxy-cidr %q", part)
				}
				if ip.To4() != nil {
					part += "/32"
				} else {
